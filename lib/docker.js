...
class Docker{
  ...
  pull(...args){
    ...
    this.execDockerCommand('pull', image, ...args, (err, output)=>{
      if(err){
        this.emit('error', err);
        if(callback){
          return callback(err);
  ...
  run(...runArgs){
    ...
    this.execDockerCommand('run', ...args, (err, output)=>{
      if(err){
        this.emit('error', err);
        if(callback){
          return callback(err);
  ...
  stop(...args){
    ...
    this.execDockerCommand('stop', this.containerId, ...args, (err, output)=>{
      if(err){
        this.emit('error', err);
        if(callback){
          return callback(err);
  ...
  kill(callback){
    ...
    this.execDockerCommand('kill', this.containerId, (err, output)=>{
      if(err){
        this.emit('error', err);
        if(callback){
          return callback(err);
  ...
  rm(...args){
    ...
    this.execDockerCommand('rm', '-v', this._lastContainerId, ...args, (err, output)=>{
      if(err){
        this.emit('error', err);
        if(callback){
          return callback(err);
  ...
  exec(...args){
    ...
    this.execDockerCommand('exec', this.containerId, command, ...args, (err, output)=>{
      if(err){
        this.emit('error', err);
        if(callback){
          return callback(err);
