...
const staticSpawn=(command, args, callback)=>{
  ...
  process.stdout.on('data', (data)=>{
    const str = data.toString();
    all.push(str);
    if(str.match(/^rpc error:/)){
      return stderr.push(str);
    }
    stdout.push(str);
  });
...
class Docker{
  constructor(opts){
    ...
    this._lastContainerId = options.containerId || '';
  ...
  status(...args){
    const callback = typeof(args[args.length?args.length-1:0])==='function'?args.pop():false;
    const containerId = this.containerId || this.lastContainerId;
    if(!containerId){
      if(callback){
    ...
    return this.execDockerCommand('ps', '-a', '--format', '"{{.ID}}\t{{.Status}}"', ...args, (err, output)=>{
      if(err){
        this.emit('error', err);
        if(callback){
          return callback(err);
      ...
      const checkForMe = (lineId)=>{
        if((!lineId)||(lineId.length<2)){
          return false;
        }
        const l = Math.min(lineId.length-1, containerId.length)
        const id = containerId.substr(0, l);
        const lid = lineId.substr(1, l);
        return id===lid;
      };
      const lines = output.stdout.join('').split('\n').filter((line, index)=>checkForMe(line.split('\t')[0]));
      if(lines.length){
        let status = lines[0].split('\t')[1];
